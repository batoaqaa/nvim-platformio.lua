...
function M.pio_refresh(callback)
  ...
  local function get_metadata(attempts, env)
    ...
    -- Helper: Internal logic to apply data to _G.metadata
    ...
    if content then
      ...
      if ok and apply_metadata(decoded) then
        vim.notify('PIO: Metadata loaded from cache', vim.log.levels.INFO)
        return
      end
    end

    -- 2. Fallback Path: Call PIO CLI
    vim.notify('PIO: Fetching metadata via CLI...', vim.log.levels.INFO)
    vim.system({ 'pio', 'project', 'metadata', '-e', active_env, '--json-output' }, { text = true }, function(obj)
      vim.schedule(function()
        ...
        -- PIO CLI wraps the env data in a table keyed by env name
        local _, data = next(raw_data or {})

        if ok and apply_metadata(data) then
          vim.notify('PIO: Metadata sync successful', vim.log.levels.INFO)
        else
          vim.notify("PIO: Failed to parse CLI output", vim.log.levels.WARN)
        end
      end)
    end)
  end
